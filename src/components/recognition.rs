// src/components/recognition.rs — Liam Abourousse Portfolio × Leptos 0.7
//
// ── UI COMPONENT ─────────────────────────────────────────────────────────────
    ...
    view! {
        <section class="recognition-section grid grid-cols-1 md:grid-cols-12 border-b">
            <div class="md:col-span-4 p-[clamp(32px,5vw,80px)] flex flex-col justify-center md:border-r">
                <h2 class="section-heading font-sans text-[0.7rem] font-semibold tracking-normal uppercase text-muted">"RECOGNITION"</h2>
                <p class="text-[0.85rem] text-muted leading-[1.7] mt-4 max-w-[360px]">
                    "Signals that matter for the profile: operated product work,
                    competitive product thinking, engineering training, and language readiness."
                </p>
            </div>
                    ...
                    {items.into_iter().map(|item| {
                        view! {
                            <div class="rec-item flex justify-between py-4.5 border-b last:border-b-0 hover-target">
                                <span class="text-[0.65rem] font-semibold tracking-normal text-muted">{item.org}</span>
